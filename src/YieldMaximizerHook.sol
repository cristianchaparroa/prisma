...
contract YieldMaximizerHook is BaseHook {
    ...
    function _afterSwap(
        ...
        // DEBUG: Emit entry event
        emit DebugSwapEntered(sender, poolId, block.number);

        // DEBUG: Check active users
        uint256 activeUsersCount = activeUsers[poolId].length;
        emit DebugActiveUsers(poolId, activeUsersCount);

        // Calculate total fees generated by this swap
        uint256 totalFeesGenerated = calculateFeesFromSwap(key, delta);

        // Only distribute fees if there are fees and active LPs
        if (totalFeesGenerated > 0 && activeUsers[poolId].length > 0) {
            // Get total liquidity in pool from all active users
            uint256 totalPoolLiquidity = getTotalPoolLiquidity(poolId);

            // DEBUG: Emit calculation details
            emit DebugSwapCalculation(poolId, totalFeesGenerated, totalPoolLiquidity, activeUsersCount);

            // CRITICAL FIX: Check for zero liquidity to prevent division by zero
            if (totalPoolLiquidity > 0) {
                // Distribute fees proportionally to all active LPs
                address[] memory users = activeUsers[poolId];

                for (uint256 i = 0; i < users.length; i++) {
                    address user = users[i];

                    // Only distribute to users with active strategies and liquidity positions
                    if (userStrategies[user].isActive && userLiquidityPositions[user][poolId].isActive) {
                        // Calculate user's proportional share of fees using safe math
                        uint256 userLiquidity = userLiquidityPositions[user][poolId].liquidityAmount;

                        // Safe division: userFeeShare = (totalFeesGenerated * userLiquidity) / totalPoolLiquidity
                        // We already checked totalPoolLiquidity > 0 above
                        uint256 userFeeShare = (totalFeesGenerated * userLiquidity) / totalPoolLiquidity;

                        // DEBUG: Emit user fee share calculation
                        emit DebugUserFeeShare(user, poolId, userLiquidity, userFeeShare, userStrategies[user].isActive);

                        if (userFeeShare > 0) {
                            // Add fees to user's accounting
                            userFees[user][poolId].totalFeesEarned += userFeeShare;
                            userFees[user][poolId].pendingCompound += userFeeShare;
                            userFees[user][poolId].lastCollection = block.timestamp;

                            emit FeesCollected(user, poolId, userFeeShare);

                            // Check if should schedule compound for this user
                ...
                // Fallback: If no tracked liquidity but users exist, give fees to swapper if they have active strategy
                // This handles the case where LP tracking isn't perfect but users have strategies
                if (userStrategies[sender].isActive) {
                    userFees[sender][poolId].totalFeesEarned += totalFeesGenerated;
                    userFees[sender][poolId].pendingCompound += totalFeesGenerated;
                    userFees[sender][poolId].lastCollection = block.timestamp;

                    emit FeesCollected(sender, poolId, totalFeesGenerated);
            ...
            // DEBUG: Emit why fees were not distributed
            if (totalFeesGenerated == 0) {
                emit DebugSwapError("No fees generated", poolId, sender);
            } else if (activeUsers[poolId].length == 0) {
                emit DebugSwapError("No active users in pool", poolId, sender);
            }
        }
